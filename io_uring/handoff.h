...
/*
 * Stash the io_uring_enter() arguments so a promoted task can resume the
 * syscall, and run pending task_work. A pending notification would
 * interrupt the blocking issues we're about to do.
 */
...
/* If true, the task is marked for handoff; pair with io_handoff_end() */
